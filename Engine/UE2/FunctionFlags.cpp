...
std::string StringifyFlags(const UEFunctionFlags flags)
{
	...
	if (flags & UEFunctionFlags::Defined) { buffer.push_back("Defined"); }
	if (flags & UEFunctionFlags::Iterator) { buffer.push_back("Iterator"); }
	if (flags & UEFunctionFlags::Latent) { buffer.push_back("Latent"); }
	if (flags & UEFunctionFlags::PreOperator) { buffer.push_back("PreOperator"); }
	if (flags & UEFunctionFlags::Singular) { buffer.push_back("Singular"); }
	if (flags & UEFunctionFlags::Net) { buffer.push_back("Net"); }
	if (flags & UEFunctionFlags::NetReliable) { buffer.push_back("NetReliable"); }
	if (flags & UEFunctionFlags::Simulated) { buffer.push_back("Simulated"); }
	...
	if (flags & UEFunctionFlags::Operator) { buffer.push_back("Operator"); }
	if (flags & UEFunctionFlags::Static) { buffer.push_back("Static"); }
	if (flags & UEFunctionFlags::HasOptionalParms) { buffer.push_back("HasOptionalParms"); }
	...
	if (flags & UEFunctionFlags::K2Call) { buffer.push_back("K2Call"); }
	if (flags & UEFunctionFlags::K2Override) { buffer.push_back("K2Override"); }
	if (flags & UEFunctionFlags::K2Pure) { buffer.push_back("K2Pure"); }
